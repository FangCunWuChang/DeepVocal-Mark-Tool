...
void Egg::paintEvent(QPaintEvent *event)
{
    ...
    painter.setPen(pen);
    font.setPixelSize(height()*0.015);
    painter.setFont(font);
    ...
    count_l++;
    painter.drawText(0.05*width(),0.2*height()+count_l*height()*0.015+count_l*height()*0.005,"版本号："+QString::number(DVMT_VERSION));
    count_l++;
    ...
    count_l++;
    painter.drawText(0.05*width(),0.2*height()+count_l*height()*0.015+count_l*height()*0.005,"#openwav - 旧版读取wav窗口");
    count_l++;
    ...
    for(int i=0;i<plugin->pluginurllist.size();i++){
        QString plt="插件名："+plugin->pluginlist.at(i);
        QString plit="插件信息："+plugin->plugininfolist.at(i);
    ...
    if(plugin->pluginurllist.size()==0){
    ...
    if(((arg1.contains("HB",Qt::CaseInsensitive)||arg1.contains("Happy Birthday",Qt::CaseInsensitive)||arg1.contains("生贺",Qt::CaseInsensitive)||arg1.contains("生日快乐",Qt::CaseInsensitive))&&(arg1.contains("休休",Qt::CaseInsensitive)))||(arg1.contains("愁杀人来关月事",Qt::CaseInsensitive)&&arg1.contains("得休休处且休休",Qt::CaseInsensitive))){
        ...
        if(!arg1.isEmpty()){
            if(arg1.at(0)=='@'){
                ...
                plnt.remove(0,1);
                for(int i=0;i<plugin->pluginurllist.size();i++){
                    if(plnt==plugin->pluginlist.at(i)){
                        id=i;
                        break;
                    }
                }
                if(id>=0&&id<plugin->pluginurllist.size()){
                    ui->line->clear();
                    openplugin(id);
...
{
    plugin->openplugin(id);
}

void Egg::setPlugins(Plugins* plu)
{
    this->plugin=plu;
}
