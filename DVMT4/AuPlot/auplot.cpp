    ...
    ui(new Ui::AuPlot)
{
    ...
    connect(ui->tool,&ToolButtons::buttonchecked,this,&AuPlot::buttonchecked);
    connect(ui->tool,&ToolButtons::buttonenabled,this,&AuPlot::buttonenabled);
    //lock();
    this->tip("无数据!");
    this->lock();
...
void AuPlot::resizeEvent(QResizeEvent *event)
{
    ...
    ui->waveview->move(0,0*height());
    ui->splitter->resize(width(),0.95*height());
    ui->splitter->move(0,0.05*height());
...
void AuPlot::setctb(QString button,bool checked)
{
    ui->tool->setctb(button,checked);
}
