...
{
    QString out="正确";
    QStringList sltc1,sltc3,slts,slte,sltn;
    for(int i=0;i<in.size();i++){
        if(!in.at(i).isEmpty()){
            ...
            if(slt.size()==3){
                ...
                if(slt.at(1)=="B"||slt.at(1)=="L"||slt.at(1)=="R"||slt.at(1)=="-"){
                    ref=true;
                    out=QString::asprintf("(第%d条)句法有误：",i+1)+in.at(i)+":\""+slt.at(1)+"\"与固有标记(\"B\",\"L\",\"R\",\"-\")重复";
                    break;
                }
                if(ref){
                    break;
