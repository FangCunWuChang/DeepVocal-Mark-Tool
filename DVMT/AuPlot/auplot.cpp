...
/*
    if(se!=nullptr){
        ...
        QFile file(QDir::currentPath()+"/temp/play.wav");
        file.remove();
    }*/
    if(se.isPlaying()){
        se.stop();
...
/*
    ...
    if(repaint){
        ...
        if(wave.channels==1&&wave.samplespersec==44100&&wave.wavedataL->size()>=2*global::sets::perp){
...
void AuPlot::draw(QPixmap wavepixv,QPixmap wavepixp,QPixmap specpix,QPixmap enepix,QPixmap rptzpix,QPixmap acfpix,QVector<double> v1,QVector<double> v2,QVector<double> v3,QVector<bool> v4,bool save)
{
    ui->waveview->setpix(wavepixv);
    ui->waveplot->setpix(wavepixp,enepix,rptzpix);
    ui->waveplot->setv(v1,v2);
    ui->specplot->setpix(specpix,acfpix);
    ...
    if(save){
        ...
        sthr->set(filepath,wavepixv,wavepixp,specpix,enepix,rptzpix,acfpix,v1,v2,v3,v4);
        sthr->start();
    }
...
void AuPlot::plotplay(bool play)
{
    if(selected){
        if(play){
            ...
            //qDebug("2");
                    ...
                    //qDebug("3");
                    wavetemp->WriteFile(QDir::currentPath()+"/temp/play.wav");

                    //qDebug("4");
                    delete wavetemp;
                    delete wave;

                    //QFile::copy(QDir::currentPath()+"/temp/play.wav",QDir::currentPath()+"/temp/play_debug.wav");

                    //qDebug("5");

                    //QMediaPlaylist pl;

                    //pl.addMedia(QUrl(QDir::currentPath()+"/temp/play.wav"));
    ...
    */
                    ...
                    se.setSource(QUrl::fromLocalFile(QDir::currentPath()+"/temp/play.wav"));
                    se.setVolume(1);
                    se.setLoopCount(0);
