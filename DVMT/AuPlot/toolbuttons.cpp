...
void ToolButtons::setplay(bool play)
{
    if(play!=ui->b6->isChecked()){
        if(play){
            ...
            emit buttonchecked("play",true);
            ui->b6->setText("停止");
        }else{
            ui->b6->setChecked(false);
            emit buttonchecked("play",false);
            ui->b6->setText("播放");
        }
    }
    ui->b6->setShortcut(QKeySequence("Alt+X"));
...
void ToolButtons::setfourlines(bool fourlines)
{
    ...
    emit buttonchecked("1",true);
    ui->b9->setChecked(false);
    emit buttonchecked("2",false);
    ui->b10->setChecked(false);
    emit buttonchecked("3",false);
    ui->b11->setChecked(false);
    emit buttonchecked("4",false);
    emit linechanged(1);
    if(fourlines){
        ui->b8->setEnabled(true);
        emit buttonenabled("1",true);
        ui->b9->setEnabled(true);
        emit buttonenabled("2",true);
        ui->b10->setEnabled(true);
        emit buttonenabled("3",true);
        ui->b11->setEnabled(true);
        emit buttonenabled("4",true);
    }else{
        ui->b8->setEnabled(true);
        emit buttonenabled("1",true);
        ui->b9->setEnabled(true);
        ...
        emit buttonenabled("3",false);
        ui->b11->setEnabled(false);
        emit buttonenabled("4",false);
    }
}
...
void ToolButtons::on_b9_clicked()
{
    ...
    ui->b10->setChecked(false);
    ui->b11->setChecked(false);
    emit buttonchecked("1",false);
    emit buttonchecked("2",true);
    emit buttonchecked("3",false);
    emit buttonchecked("4",false);
    emit linechanged(2);
}
...
void ToolButtons::setlocked(bool locked)
{
    ...
    emit buttonenabled("rea",!locked);
    emit buttonenabled("cle",!locked);
    emit buttonenabled("save",!locked);
}
...
void ToolButtons::setctb(QString button,bool checked)
{
    if(button=="save"){
        emit saved();
    }else if(button=="auto"){
        ui->b7->setChecked(checked);
        this->Auto=checked;
        emit autoed(checked);
    }else if(button=="play"){
        ui->b6->setChecked(checked);
        on_b6_clicked(checked);
    }else if(button=="1"){
        on_b8_clicked();
    }else if(button=="2"){
        on_b9_clicked();
        ...
        ui->b1->setChecked(checked);
        on_b1_clicked(checked);
    }else if(button=="rptz"){
        ui->b2->setChecked(checked);
        on_b2_clicked(checked);
    }else if(button=="rea"){
        on_b4_clicked();
    }else if(button=="cle"){
        on_b5_clicked();
